...
if __name__ == '__main__':
    ...
    for i, v in enumerate(imgs_path):
        #if the file is a DS_Store file, skip it
        if v.split('/')[-1] == '.DS_Store':
            continue
        ...
        #change directory 
        os.chdir('../../../')
        cv2.imwrite(os.path.join('.', 'outputs/nn-depth.png'), (pred_depth_ori/pred_depth_ori.max() * 60000).astype(np.uint16))
